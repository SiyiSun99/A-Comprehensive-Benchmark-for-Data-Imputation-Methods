...
class Featurize(nn.Module):
    ...
    def forward(self, x_num, tokens):
        ...
        # print(x_num.shape) # 128,7
        # print(f_num.shape) # 128,32
        f_emb = self.embed(tokens)
        # print(tokens.shape) # 128,8
        # print(f_emb.shape) # 128,8,32
        # f_int = self.attn(f_emb) # remove attention block
        # print(f_int.shape) # 128,8,32
        f_int = f_emb.flatten(start_dim=1)
        # print(f_int.shape) # 128, 256
        f_tot = torch.cat([f_num, f_int], dim=1)
        # print(f_tot.shape) # 128, 288
        return f_tot
...
if __name__ == '__main__':
    ...
    for m_m in tqdm(missing_mechanism):
        for m_r in tqdm(missing_ratio):
            folder_path = '/data/coml-data-imputation/shug7754/data_stored/data_dsn/{}/Case{}/{}'.format(m_m, casenum, m_r)
            # 10 sample missing dataset
            for smp in tqdm(range(10)):
                i_fname = '/data/coml-data-imputation/shug7754/data_stored/data_miss/{}/Case{}/{}/{}.csv'.format(m_m, casenum, m_r, smp)
                res_csv_path = '/data/coml-data-imputation/shug7754/data_stored/data_dsn/{}/Case{}/{}/{}.csv'.format(m_m, casenum, m_r, smp)
